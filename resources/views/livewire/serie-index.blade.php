<section class="container mx-auto p-6 font-mono">
	...
	<div class="w-full flex mb-4 p-2 justify-end">
		<form class="flex space-x-4 shadow bg-white rounded-md m-2 p-2">
			<div class="p-1 flex items-center">
				<label for="tmdbId" class="block text-sm font-medium text-gray-700 md:mr-4">Serie Tmdb Id</label>
				<div class="relative rounded-md shadow-sm">
					<input wire:model="tmdbId" id="tmdbId" name="tmdb_id_g" class="px-3 py-2 border border-gray-300 rounded"
						placeholder="Serie TMDB ID" />
				</div>
			</div>
			<div class="p-1">
	...
	<x-jet-dialog-modal wire:model="showSerieModal">
		...
		<x-slot name="footer">
			<x-m-button wire:click="closeSerieModal" class="bg-gray-600 hover:bg-gray-800 text-white">Cancel</x-m-button>
			<x-m-button wire:click="updateSeries">Update</x-m-button>
		</x-slot>
	</x-jet-dialog-modal>
	{{-- modal end --}}
</section>
