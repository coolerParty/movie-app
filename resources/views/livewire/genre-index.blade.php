<section class="container mx-auto p-6 font-mono">
	<div class="w-full flex mb-4 p-2 justify-end">
		<form class="flex space-x-4 shadow bg-white rounded-md m-2 p-2">
			<div class="p-1 flex items-center">
				<label for="tmdbId" class="block text-sm font-medium text-gray-700 md:mr-4">Genre Tmdb Id</label>
				<div class="relative rounded-md shadow-sm">
					<input wire:model="tmdbId"  id="tmdbId" name="tmdb_id_g" class="px-3 py-2 border border-gray-300 rounded"
						placeholder="Genre TMDB ID" />
				</div>
			</div>
			<div class="p-1">
					...
					@foreach ($genres as $genre)
						<tr class="text-gray-700">
							...
							<td class="px-4 py-3 text-sm border">
								<x-m-button wire:click="showEditModal({{ $genre->id }})" class="bg-green-500 hover:bg-green-700 text-white">
									Edit</x-m-button>
								<x-m-button wire:click="deleteGenre({{ $genre->id }})" class="bg-red-500 hover:bg-red-700 text-white">Delete
								</x-m-button>
							</td>
						</tr>
					@endforeach
	...
	<x-jet-dialog-modal wire:model="showGenreModal">
							...
							<div class="px-4 py-5 bg-white sm:p-6">
								<div class="grid grid-cols-6 gap-6">
									<div class="col-span-6 sm:col-span-3">
										...
										<input wire:model="title" id="title" type="text" autocomplete="given-name"
		...
		<x-slot name="footer">
			<x-m-button wire:click="closeGenreModal" class="bg-gray-600 hover:bg-gray-800 text-white">Cancel</x-m-button>
